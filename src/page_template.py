...
<!-- Social card. Without these a shared link has nothing to build a preview
     from but <title>, and renders as a generic card — which is exactly what
     happened the first time this URL was sent to someone.

     The image is the site's canonical module card, generated by the site
     repo's tools/build_thumbs.py from a single MODULES table so every card in
     the portfolio shares one generator. It is referenced rather than copied:
     that file's own note is that CI regenerates the images in place so they
     are never hand-carried between machines, and vendoring a second copy here
     would fork the one thing that keeps the family consistent.

     The dependency is cross-repo and worth naming: if that asset is renamed or
     removed, this card degrades silently. It is same-origin, so it cannot
     break from a CORS or mixed-content change. -->
...
<meta property="og:image" content="https://www.robbinsanalytics.com/assets/thumb-controltower.png">
...
<meta name="twitter:image" content="https://www.robbinsanalytics.com/assets/thumb-controltower.png">
